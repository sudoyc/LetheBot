...
const ACCOUNT_TYPE_LABELS = { private: 'Private account', group_member: 'Group member', temp_session: 'Temporary session' };
const ACCOUNT_STATUS_LABELS = { active: 'Active', disabled: 'Disabled' };
...
const controls = append(view, 'form', { class: 'memory-scope-band governance-control-grid', 'aria-labelledby': 'identity-controls-title' });append(controls, 'h2', { id: 'identity-controls-title' }, 'Exact QQ account');
const accountControl = append(controls, 'div');append(accountControl, 'label', { for: 'identity-account-input' }, 'QQ account ID');const input = append(accountControl, 'input', { id: 'identity-account-input', type: 'password', inputmode: 'numeric', autocomplete: 'off', required: '', minlength: '5', maxlength: '12', pattern: '[1-9][0-9]{4,11}', 'aria-describedby': 'identity-account-help' });append(accountControl, 'small', { id: 'identity-account-help', class: 'memory-scope-note' }, 'Enter 5 to 12 digits, starting with 1–9. Used only for this preview and cleared immediately after the request.');
const previewButton = append(controls, 'button', { id: 'identity-preview-button', class: 'button button-primary', type: 'submit', 'aria-controls': 'identity-preview' }, 'Preview account unlink');
...
async function preview() { const platformAccountId = input.value;if (!QQ_ACCOUNT.test(platformAccountId)) { input.setCustomValidity('Enter 5 to 12 digits, starting with 1–9.');input.reportValidity();return; }input.setCustomValidity('');clear();setHidden(success, true);setHidden(unavailable, true);const sequence = ++previewSequence;previewButton.disabled = true;const response = await mutate(UNLINK_ENDPOINT, { action: 'unlink', platform: 'qq', platformAccountId });input.value = '';if (sequence !== previewSequence) return;previewButton.disabled = false;if (response.status === 401) return expired();const normalized = response.status === 201 ? unlinkPreview(response.body, Date.now()) : null;if (!normalized) { setHidden(unavailable, false);announce('Platform account unlink preview unavailable.');return; }rows(previewTable, [['Account', 'Exact QQ account', ACCOUNT_TYPE_LABELS[normalized.account.accountType]], ['Transition', ACCOUNT_STATUS_LABELS[normalized.account.status], ACCOUNT_STATUS_LABELS[normalized.expected.status]], ['Consequences', 'Future identity resolution is blocked', 'The account mapping remains for privacy obligations'], ['Boundary', 'Relinking is not available in this console', 'No rollback; confirm before preview expiry']]);authority = { resourceHandle: normalized.resourceHandle, previewHandle: normalized.previewHandle, accountFingerprint: normalized.account.fingerprint, expiresAt: normalized.previewExpiresAt };setHidden(previewSurface, false);confirmButton.disabled = false;timer = window.setTimeout(() => { authority = null;confirmButton.disabled = true;previewButton.disabled = false;setHidden(previewSurface, true);announce('Account unlink preview expired. Request a fresh preview.'); }, Math.min(normalized.previewExpiresAt - Date.now(), 2_147_483_647));announce('Platform account unlink preview ready. Confirmation is available until the preview expires.'); }
async function confirm() { const retained = authority;if (!retained || confirmButton.disabled || retained.expiresAt <= Date.now()) { clear();return; }authority = null;confirmButton.disabled = true;if (timer !== null) window.clearTimeout(timer);timer = null;const sequence = ++confirmSequence;const response = await mutate(UNLINK_ENDPOINT + '/confirm', { confirm: true, resourceHandle: retained.resourceHandle, previewHandle: retained.previewHandle });if (sequence !== confirmSequence) return;if (response.status === 401) return expired();const normalized = response.status === 200 ? unlinkResult(response.body, retained.accountFingerprint) : null;if (!normalized) { setHidden(previewSurface, true);setHidden(unavailable, false);previewButton.disabled = false;announce('Platform account unlink confirmation unavailable. Request a fresh preview.');return; }rows(resultTable, [['Account', 'Exact QQ account', ACCOUNT_TYPE_LABELS[normalized.account.accountType]], ['State', ACCOUNT_STATUS_LABELS[normalized.account.status], 'Unlinked'], ['Evidence', 'Audit event recorded', normalized.unlinkedAt], ['Boundary', 'Relinking is not available in this console', 'Future identity resolution is blocked']]);setHidden(previewSurface, true);setHidden(success, false);previewButton.disabled = false;announce('Platform account unlinked.'); }
input.addEventListener('input', () => { input.setCustomValidity('');clear();setHidden(success, true);setHidden(unavailable, true); });controls.addEventListener('submit', (event) => { event.preventDefault();void preview(); });confirmButton.addEventListener('click', () => void confirm());setHidden(unavailable, true);setHidden(success, true);
return { nav, view, load: () => { setHidden(view, false); }, reset: () => { input.value = '';clear();resultTable.replaceChildren();setHidden(unavailable, true);setHidden(success, true);setHidden(view, true); } };
}
...
const controls = append(view, 'section', { class: 'memory-scope-band governance-control-grid', 'aria-labelledby': 'operations-actions-title' });append(controls, 'h2', { id: 'operations-actions-title' }, 'Governed actions');const backupButton = append(controls, 'button', { id: 'operations-backup-preview-button', class: 'button button-primary', type: 'button', 'aria-controls': 'operations-preview' }, 'Preview verified backup');const restoreButton = append(controls, 'button', { id: 'operations-restore-preview-button', class: 'button button-secondary', type: 'button', disabled: '', 'aria-controls': 'operations-preview' }, 'Preview restore handoff');
...
async function load() {
  const sequence = ++readSequence;clear();retentionPanel.clear();setHidden(success, true);setHidden(unavailable, true);setHidden(current, true);refresh.disabled = true;
  const response = await requestJson(OPERATIONS_ENDPOINT);if (sequence !== readSequence) return;refresh.disabled = false;
  if (response.status === 401) return expired();const normalized = response.status === 200 ? operationsStatus(response.body) : null;
  if (!normalized) { setHidden(unavailable, false);announce('Operations unavailable.');return; }
  rows(currentTable, [['Overall', normalized.overall, normalized.generatedAt], ['Database', normalized.database.integrity, normalized.database.foreignKeys], ['Schema', normalized.schema.ready ? 'ready' : 'attention required', String(normalized.schema.missingTableCount) + ' missing'], ['Retention', String(normalized.configuration.retentionDays.rawEvents) + ' raw-event days', 'Policy evidence only']]);setHidden(unavailable, true);setHidden(current, false);announce('Operations updated.');
}
async function preview(kind) {
  clear();retentionPanel.clear();setHidden(success, true);setHidden(unavailable, true);backupButton.disabled = true;restoreButton.disabled = true;const sequence = ++previewSequence;
  const endpoint = kind === 'backup' ? OPERATIONS_ENDPOINT : RESTORE_ENDPOINT;const body = kind === 'backup' ? { action: 'create_verified_backup' } : { action: 'prepare_restore_handoff', backupRef };
  const response = await mutate(endpoint, body);if (sequence !== previewSequence) return;updateActions();if (response.status === 401) return expired();
  const normalized = response.status === 201 ? (kind === 'backup' ? backupPreview(response.body, Date.now()) : restorePreview(response.body, Date.now())) : null;
  if (!normalized) { setHidden(unavailable, false);announce('Operations preview unavailable.');return; }
  if (kind === 'backup') rows(previewTable, [['Action', kind === 'backup' ? 'Create verified backup' : 'Prepare restore handoff', kind === 'backup' ? 'Available' : 'Verified backup available'], ['Effects', 'Private artifact creation', 'No database mutation'], ['Restore', 'Stopped service only', 'Available after completion'], ['Boundary', 'Artifact removal is not exposed', 'No artifact removal']]);
  else rows(previewTable, [['Action', kind === 'backup' ? 'Create verified backup' : 'Prepare restore handoff', kind === 'backup' ? 'Available' : 'Verified backup available'], ['Effects', 'No restore execution', 'Service stop required'], ['Restore', 'Stopped service only', String(normalized.artifact.sizeBytes) + ' bytes'], ['Boundary', 'No in-process restore effect', 'Handoff only']]);
  authority = { kind, handle: normalized.previewHandle, expiresAt: normalized.previewExpiresAt, backupRef };backupButton.disabled = true;restoreButton.disabled = true;confirmButton.textContent = kind === 'backup' ? 'Confirm verified backup' : 'Prepare restore handoff';setHidden(previewSurface, false);confirmButton.disabled = false;
  timer = window.setTimeout(() => { authority = null;confirmButton.disabled = true;updateActions();setHidden(previewSurface, true);announce('Operations preview expired. Request a fresh preview.'); }, Math.min(normalized.previewExpiresAt - Date.now(), 2_147_483_647));announce('Operations preview ready. Confirmation is available until the preview expires.');
}
async function confirm() {
  const retained = authority;if (!retained || confirmButton.disabled || retained.expiresAt <= Date.now() || (retained.kind === 'restore' && retained.backupRef !== backupRef)) { clear();return; }
  ...
  const endpoint = retained.kind === 'backup' ? OPERATIONS_ENDPOINT + '/confirm' : RESTORE_ENDPOINT + '/confirm';const body = retained.kind === 'backup' ? { confirm: true, previewHandle: retained.handle } : { confirm: true, previewHandle: retained.handle, backupRef: retained.backupRef };
  const response = await mutate(endpoint, body);if (sequence !== confirmSequence) return;updateActions();if (response.status === 401) return expired();
  if (retained.kind === 'backup') {
    const normalized = response.status === 200 ? backupResult(response.body) : null;if (!normalized) { setHidden(previewSurface, true);setHidden(unavailable, false);announce('Verified backup confirmation unavailable. Request a fresh preview.');return; }
    backupRef = normalized.backupRef;restoreButton.disabled = backupRef === null;rows(resultTable, [['Status', normalized.status, normalized.artifact.integrity], ['Artifact', String(normalized.artifact.sizeBytes) + ' bytes', normalized.pages.complete ? 'Complete' : 'Incomplete'], ['Restore', 'Stopped service only', normalized.restore.available ? 'Handoff available' : 'Unavailable'], ['Boundary', 'Private server-owned artifact', 'Reference retained in memory only']]);announce(normalized.status === 'completed' ? 'Verified backup completed.' : 'Verified backup needs attention.');
  } else {
    const normalized = response.status === 200 ? handoff(response.body) : null;if (!normalized) { setHidden(previewSurface, true);setHidden(unavailable, false);announce('Restore handoff confirmation unavailable. Request a fresh preview.');return; }
    backupRef = null;restoreButton.disabled = true;rows(resultTable, [['Status', normalized.status, normalized.expiresAt || 'No handoff created'], ['Artifact', 'Verified backup retained', 'No artifact mutation'], ['Restore', 'Stopped service only', 'No in-process restore'], ['Boundary', 'Stop the service before execution', normalized.effects.restoreExecution ? 'Unexpected execution' : 'Handoff only']]);announce(normalized.status === 'pending' ? 'Restore handoff prepared.' : 'Restore handoff needs attention.');
  }
  setHidden(unavailable, true);setHidden(previewSurface, true);setHidden(success, false);
}
