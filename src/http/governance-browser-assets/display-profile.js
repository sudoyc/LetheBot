...
const controls = append(view, 'section', { class: 'memory-scope-band', 'aria-labelledby': 'display-profile-scope-title' });append(controls, 'h2', { id: 'display-profile-scope-title' }, 'Exact user scope');const select = append(controls, 'select', { id: 'display-profile-scope-select', disabled: '' });append(select, 'option', { value: '' }, 'Select a user scope');
...
const detailSurface = append(view, 'section', { class: 'worker-heartbeats-content', hidden: '' });const displayDetailTable = detailTable(detailSurface, 'Display target', ['Target', 'Current display', 'History', 'Boundary']);const previewButton = append(detailSurface, 'button', { id: 'display-profile-preview-button', class: 'button button-primary', type: 'button', disabled: '' }, 'Preview redaction');
...
function clearDetail() { detailSequence += 1;clearAuthority();selectedTarget = null;selectedDetail = null;displayDetailTable.replaceChildren();setHidden(detailSurface, true); }
async function loadDetail(entry) { const selectedScope = scope();if (!selectedScope) return;clearDetail();selectedTarget = entry;const sequence = ++detailSequence;setHidden(loading, false);const response = await requestJson(TARGETS_ENDPOINT + '/' + entry.handle, { headers: { 'X-LetheBot-Scope': selectedScope.handle } });if (sequence !== detailSequence) return;setHidden(loading, true);if (response.status === 401) return expired();const normalized = response.status === 200 ? detail(response.body, entry) : null;if (!normalized) { setHidden(unavailable, false);announce('Display target unavailable.');return; }selectedDetail = normalized;rows(displayDetailTable, [['Target', normalized.target.label, normalized.target.targetKind], ['Current display', normalized.currentDisplay ? normalized.currentDisplay.value : 'No current display', normalized.currentDisplay?.redacted ? 'Redacted' : 'Bounded value'], ['History', String(normalized.nicknameHistory.length), normalized.nicknameHistoryTruncated ? 'First 100 only' : normalized.target.history.lifecycle], ['Boundary', 'Redaction is irreversible', normalized.target.fingerprint]]);setHidden(unavailable, true);setHidden(detailSurface, false);previewButton.disabled = false;announce('Display target updated.'); }
async function loadTargets() { const selectedScope = scope();clearDetail();entries = [];targets.replaceChildren();if (!selectedScope) { setHidden(content, true);setHidden(empty, false);return; }const sequence = ++listSequence;setHidden(loading, false);const response = await requestJson(TARGETS_ENDPOINT, { headers: { 'X-LetheBot-Scope': selectedScope.handle } });if (sequence !== listSequence) return;setHidden(loading, true);if (response.status === 401) return expired();const normalized = response.status === 200 ? page(response.body, Date.now()) : null;if (!normalized) { setHidden(unavailable, false);announce('Display targets unavailable.');return; }entries = normalized.entries;for (const [index, entry] of entries.entries()) { const button = append(targets, 'button', { class: 'memory-record-row', type: 'button', 'data-display-index': String(index), 'aria-expanded': 'false' });append(button, 'span', { class: 'memory-record-primary' }, entry.label);append(button, 'span', { class: 'memory-record-secondary' }, entry.currentProfile.present ? 'Profile present' : 'History only'); }setHidden(unavailable, true);setHidden(empty, entries.length !== 0);setHidden(content, entries.length === 0);announce(entries.length ? 'Display targets updated.' : 'No display targets.'); }
async function loadCatalog() { const sequence = ++catalogSequence;listSequence += 1;clearDetail();refresh.disabled = true;select.disabled = true;setHidden(loading, false);const response = await requestJson(SCOPES_ENDPOINT);if (sequence !== catalogSequence) return;refresh.disabled = false;setHidden(loading, true);if (response.status === 401) return expired();const normalized = response.status === 200 ? catalog(response.body, Date.now()) : null;if (!normalized) { setHidden(unavailable, false);announce('Display scopes unavailable.');return; }scopes = normalized.entries;select.replaceChildren(createElement('option', { value: '' }, 'Select a user scope'));for (const [index, entry] of scopes.entries()) append(select, 'option', { value: String(index + 1) }, entry.label);select.disabled = scopes.length === 0;setHidden(unavailable, true);setHidden(content, true);setHidden(empty, false);announce('Display scopes updated.'); }
async function loadPreview() { const selectedScope = scope();if (!selectedScope || !selectedTarget || !selectedDetail || previewButton.disabled) return;clearAuthority();const sequence = ++previewSequence;previewButton.disabled = true;const response = await mutate(TARGETS_ENDPOINT + '/' + selectedTarget.handle, selectedScope.handle, { action: 'redact' });if (sequence !== previewSequence) return;if (response.status === 401) return expired();const normalized = response.status === 201 ? preview(response.body, selectedTarget, selectedScope, Date.now()) : null;if (!normalized) { previewButton.disabled = false;announce('Display redaction preview unavailable.');return; }rows(previewTable, [['Target', normalized.target.label, normalized.target.targetKind], ['Rows', String(normalized.expected.affectedRows.total), String(normalized.current.openNicknameHistoryRows) + ' open intervals'], ['Consequences', normalized.expected.privacyConsequences.join(', '), normalized.expected.durableEffects.join(', ')], ['Boundary', normalized.rollback.boundary, 'No rollback supported']]);authority = { scope: { ...selectedScope }, target: { ...selectedTarget }, handle: normalized.handle, expiresAt: normalized.expiresAt };setHidden(previewSurface, false);confirmButton.disabled = false;timer = window.setTimeout(() => { authority = null;confirmButton.disabled = true;setHidden(previewSurface, true);announce('Display redaction preview expired.'); }, Math.min(normalized.expiresAt - Date.now(), 2_147_483_647));announce('Display redaction preview ready.'); }
async function confirm() { const retained = authority;const selectedScope = scope();if (!retained || confirmButton.disabled || !selectedScope || !selectedTarget || selectedScope.handle !== retained.scope.handle || selectedTarget.handle !== retained.target.handle || retained.expiresAt <= Date.now()) { clearAuthority();return; }authority = null;confirmButton.disabled = true;if (timer !== null) window.clearTimeout(timer);timer = null;const sequence = ++confirmSequence;const response = await mutate(TARGETS_ENDPOINT + '/' + retained.target.handle + '/confirm', selectedScope.handle, { confirm: true, previewHandle: retained.handle });if (sequence !== confirmSequence) return;if (response.status === 401) return expired();const normalized = response.status === 200 ? result(response.body, retained.target) : null;if (!normalized) { setHidden(previewSurface, true);previewButton.disabled = !selectedDetail;announce('Display redaction confirmation unavailable.');return; }rows(resultTable, [['Target', normalized.target.label, normalized.outcome], ['Rows', String(normalized.affectedRows.total), String(normalized.openNicknameHistoryRowsClosed) + ' intervals closed'], ['Evidence', normalized.evidence.auditEvent, normalized.redactedAt], ['Boundary', normalized.rollback.boundary, normalized.privacyConsequences.join(', ')]]);setHidden(success, false);setHidden(previewSurface, true);clearDetail();void loadTargets();announce('Display data redacted.'); }
select.addEventListener('change', () => void loadTargets());refresh.addEventListener('click', () => void loadCatalog());targets.addEventListener('click', (event) => { const button = event.target?.closest?.('[data-display-index]');if (!button || !targets.contains(button)) return;const entry = entries[Number(button.getAttribute('data-display-index'))];if (entry) void loadDetail(entry); });previewButton.addEventListener('click', () => void loadPreview());confirmButton.addEventListener('click', () => void confirm());setHidden(loading, true);setHidden(unavailable, true);setHidden(empty, true);setHidden(success, true);
return { nav, view, load: loadCatalog, reset: () => { catalogSequence += 1;listSequence += 1;clearDetail();scopes = [];entries = [];select.replaceChildren(createElement('option', { value: '' }, 'Select a user scope'));select.disabled = true;targets.replaceChildren();resultTable.replaceChildren();setHidden(content, true);setHidden(empty, true);setHidden(success, true);setHidden(view, true); } };
}
