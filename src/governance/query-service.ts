...
} from '../storage/context-trace-repository.js';
import {
...
type RestorableMemoryRecordState = Extract<
  MemoryRecord['state'],
...
const MEMORY_PROVENANCE_ACTOR_CLASSES = [
  'user',
  'evaluator',
  'tool',
  'worker',
...
interface ExplainTurnResolutionRow {
  id: string;
...
function describeGovernanceActionExecutionEffect(
...
function parseGovernanceJson(text: string): unknown {
  try {
...
function redactGovernanceStringArray(values: string[]): string[] {
  return values.map((value) => redactGovernanceDisplayString(value).text);
}

function projectBoundedGovernanceText(
  text: string,
...
function sameMemoryMaintenanceExactScope(
