...
describe('E2E: Full Memory Cycle', () => {
  ...
  it('should handle conversation history in context', async () => {
    ...
    for (const msg of messages) {
      ...
      const eventId = `evt-${msg.id}`;
      db.prepare(`
        INSERT INTO raw_events (
          id, type, timestamp, source, conversation_id, payload, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(
        eventId,
        'message.received',
    ...
    // 构建上下文时传入 recentMessages（ContextBuilder 不从数据库读取历史）
    const recentMessages = messages.map(msg => ({
      messageId: msg.id,
      senderId: msg.isBot ? 'bot' : userId,
      text: msg.text,
      timestamp: new Date(),
      isFromBot: msg.isBot,
    }));
